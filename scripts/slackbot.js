...
module.exports = function(robot) {
  ...
  //  Example
   bot.hear(/Good morning!/i, function(msg) {
     return msg.send("Did you have breakfast?");
   });

   bot.hear(/Hello!/i, function(res) {
     return res.send("Did you eat?");
   });

   robot.respond(/Did you eat salad today?/i, function(res) {
     return res.send("Of course, I love salad!");
   });

   bot.respond(/I love to eat (.*) for breakfast/i, function(msg) {
     var breakfast;
     breakfast = msg.match[1];
     if (drinkName === "croisant and coffee"){
       return msg.send("I don't eat breakfast!");
     } else {
       return msg.reply("I love to eat " + breakfast + "!");
     }
